    ...
    <div style="padding: 0 16px 0 16px; margin-top: 6px;">
        <table class="info-table">
            <tr>
                ...
                <td class="val">{{ $salary->staff->name }}</td>
                <td class="lbl">Employee ID</td>
                <td class="val">{{ $salary->staff->employee_id }}</td>
            </tr>
            <tr>
                <td class="lbl">Category</td>
                <td class="val">{{ $salary->staff->category }}</td>
                <td class="lbl">Joining Date</td>
                <td class="val">{{ \Carbon\Carbon::parse($salary->staff->joining_date)->format('d M Y') }}</td>
            </tr>
            <tr>
                <td class="lbl">UAN Number</td>
                <td class="val">{{ $salary->staff->UAN ?? '—' }}</td>
                <td class="lbl">ESIC Number</td>
                <td class="val">{{ $salary->staff->ESIC_Number ?? '—' }}</td>
            </tr>
            <tr>
                <td class="lbl">Bank Name</td>
                <td class="val">{{ $salary->staff->bank->name ?? '—' }}</td>
                <td class="lbl">Account No.</td>
                <td class="val">{{ $salary->staff->Account_Number ?? '—' }}</td>
            </tr>
            <tr>
                <td class="lbl">IFSC Code</td>
                <td class="val">{{ $salary->staff->IFSC ?? '—' }}</td>
