...
<div class="page-header">
    <div>
        <div class="page-title">Edit Labour</div>
        <div class="page-subtitle">Update labour information</div>
    </div>
...
<div class="card" style="max-width:800px;">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('labours.update', $labour->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
...
{{-- New Section for getting employee id automatically when name is entered --}}
            <div class="form-grid-2">
                ...
                <div class="form-group">
                    ...
                    <input
                        ...
                        id="employee_id"
                        name="employee_id"
                        value="{{ old('employee_id', $labour->employee_id) }}"
                        readonly
                        required
            ...
            <div class="form-group">
                <label>Address</label>
                <textarea name="address" rows="2" placeholder="Full address">{{ old('address', $labour->address) }}</textarea>
            </div>
    ...
    <div class="form-grid-3">
        ...
        <div class="form-group">
            ...
            <select id="working_days" name="working_days" required>
                ...
                @for($i = 26; $i <= 31; $i++)
                    <option value="{{ $i }}" {{ old('working_days', $labour->working_days) == $i ? 'selected' : '' }}>{{ $i }} Days</option>
                @endfor
            </select>
        </div>
            ...
            <div class="form-group">
                ...
                {{-- <select id="working_hours" name="working_hours" required> --}}
                    ...
                    @for($i = 8; $i <= 13; $i++)
                        <option value="{{ $i }}" {{ old('working_hours_per_day', $labour->working_hours_per_day) == $i ? 'selected' : '' }}>{{ $i }} Hours</option>
                    @endfor
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="ot_rate_multiplier" name="ot_rate_multiplier" required>
                    ...
                    <option value="1.5" {{ old('ot_rate_multiplier', $labour->ot_rate_multiplier) == '1.5' ? 'selected' : '' }}>1.5x</option>
                    <option value="2.0" {{ old('ot_rate_multiplier', $labour->ot_rate_multiplier) == '2.0' ? 'selected' : '' }}>2x</option>
                </select>
            </div>
    ...
    <div class="form-group">
        ...
        <select name="status"
            ...
            <option value="active"
                {{ old('status', $labour->status) == 'active' ? 'selected' : '' }}>
            ...
            <option value="inactive"
                {{ old('status', $labour->status) == 'inactive' ? 'selected' : '' }}>
    ...
    <div class="form-group">
        ...
        <select name="relation" id="nominee_relation" value="{{ old('relation', $labour->relation) }}">
